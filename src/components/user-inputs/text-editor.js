...
export const EditorContent = ({ editorRef, handleInput }) => {
  return (
    <div className={styles.editorContent}>
      <div className={styles.contentArea} ref={editorRef} contentEditable={true} onInput={handleInput}></div>
    </div>
  );
};

export const ToolButton = ({ isActive, children, onClick }) => {
  return (
...
export const EditorToolbar = ({ tools, formatText, toggleHeading, activeFormats, insertImage, insertVideo, insertLink }) => {
...
const TextEditor = ({ children, maxW, initialContent = "", onSubmit }) => {
  ...
  const uploadFile = async (file, type) => {
    ...
    formData.append("data", JSON.stringify({ secret: userData.token_activation, type }));
  ...
  const addPlaceholder = (type) => {
    ...
    const placeholderHtml = `<div id="${id}" class="placeholder">
      <button type="button" onclick="triggerFileSelection('${id}', '${type}')">Upload ${type}</button>
      <button type="button" onclick="embedLink('${id}', '${type}')">Embed Link</button>
      <progress style="display: none;"></progress>
    </div>`;
    document.execCommand("insertHTML", false, placeholderHtml);
    setPlaceholders([...placeholders, { id, type }]);
  };

  const triggerFileSelection = (id, type) => {
    ...
    fileInput.onchange = async (e) => {
      ...
      if (file) {
        ...
        const progressBar = placeholder.querySelector("progress");
        ...
        try {
          ...
          if (fileUrl) {
            const mediaHtml = type === "image" ? `<img src="${fileUrl}" loading="lazy" />` : `<video controls src="${fileUrl}"></video>`;
  ...
  const embedLink = (id, type) => {
    const url = prompt(`Enter the ${type} URL`);
    if (url) {
      const placeholder = document.getElementById(id);
      const mediaHtml = type === "image" ? `<img src="${url}" />` : `<video controls src="${url}"></video>`;
      placeholder.outerHTML = mediaHtml;
    }
  };
  ...
  //     if (file) {
  //       try {
  //         const fileUrl = await uploadFile(file, "vid");
  //         if (fileUrl) {
  //           const videoHtml = `<video controls src="${fileUrl}"></video>`;
  //           document.execCommand("insertHTML", false, videoHtml);
  //         }
  //       } catch (error) {
  //         console.error(error);
  ...
  // const insertImage = () => addPlaceholder("image");
  // const insertVideo = () => addPlaceholder("video");
  ...
  // useEffect(() => {
  //   window.triggerFileSelection = triggerFileSelection;
  //   window.embedLink = embedLink;
  // }, []);

  return (
    <form onSubmit={handleSubmit} style={editorstyles}>
      {React.Children.map(children, (child) => {
        return React.cloneElement(child, { editorRef, formatText, toggleHeading, activeFormats, insertImage, insertVideo, insertLink, handleInput });
      })}
    </form>
  );
};
