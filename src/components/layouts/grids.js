...
export const News3Grid = ({ id, posts = [] }) => {
  ...
  return (
    <section id={compid} className={styles.news3grid}>
      {slicedposts.length > 0 && <NewsDisplayCard id={`${compid}-${slicedposts[0].id}`} title={slicedposts[0].judul_berita} short={slicedposts[0].isi_berita.substring(0, 160)} tag={slicedposts[0].nama_kategori_berita} count={slicedposts[0].counter} image={slicedposts[0].img_berita} loc={slicedposts[0].penulis_berita} date={slicedposts[0].tanggal_berita} align="stretch" height="var(--pixel-270)" onClick={() => navigate(`/berita/${slicedposts[0].slug}`)} />}
