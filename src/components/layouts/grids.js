      ...
      <div className={styles.news3gridSmall}>
        {slicedposts.slice(1).map((post, index) => (
          <div key={index} className={styles.news3grid200}>
            <NewsDisplayCard id={`${compid}-${post.id}`} title={post.judul_berita} short={post.isi_berita.substring(0, 160)} tag={post.nama_kategori_berita} image={`https://pifa.co.id/img_berita/${post.img_berita}`} loc={post.penulis_berita} date={post.tanggal_berita} align="stretch" height="var(--pixel-270)" onClick={() => navigate(`/berita/${post.slug}`)} />
          </div>
        ))}
      </div>
    </div>
