    ...
    <section id={compid} className={styles.newsSection}>
      ...
      <div className={styles.sectionBody}>
        {filteredposts.length > 0 && (
          <Fragment>
            <Container300>
              <NewsDisplayCard id={`${compid}-${filteredposts[0].id}`} title={filteredposts[0].title} short={filteredposts[0].short} tag={filteredposts[0].tag} image={filteredposts[0].image} loc={filteredposts[0].location} date={filteredposts[0].date} height={width < 464 ? "var(--pixel-270)" : "var(--pixel-450)"} flex="1" />
            </Container300>
            <Container300>
